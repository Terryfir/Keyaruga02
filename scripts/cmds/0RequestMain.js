       ...
       const p = getPrefix(event.threadID);
		...
		if (fs.existsSync(pendingIDsPath)) {
			...
			if (pendingIDs.includes(threadID)) {
				const pendingMsg = "Your request to use bot main cmds is already in pending.... \nPlease contact Certified Liar for fast approval.\n\n Facebook:https://www.facebook.com/profile.php?id=100081631170408\n\n also you can join  Terry support Box for help\nType : !support \nto join.";
...
for (const adminID of config.adminBot) {
  api.sendMessage(msg, adminID);
		...
		const notifyMsg = `✅Your approval request for using Bot's main cmds has been sent to admin Keyaru with this\nmessage: ${userMessage}.\n\n I will notify you if your thread is approved. Please wait until then.\n\n join the Terry support Box if you facing any problem\nType : !support \nto join.`;
